classdef KMCGmem < AbstractApproximationAlgorithm
    % memory efficient version of KMCG
    properties
        kXs;
        kXsys;
        dkXsXs;
    ...
    methods
        function obj = KMCGmem(cg_impl)
            obj.cg_impl = cg_impl;
        end
        
        function name = getName(obj)
            name = ['hybrid_mem_'  obj.cg_impl.getName()];
        end
    ...
    methods(Access=protected)        
        ...
        function [msg, t, times, t_pred, t_vfe, ymu_hat, ys2_hat, nlZ_hat, penalty, residual, sol_dist] = next_step_impl(obj, record_step)
            ...
            [msg, cg_cum_time, overhead_time, copy_time, S, ~, SY, cholSY] = obj.execute_next_cg_step(j);
            if msg ~= 0
            ...
            t_pred0 = tic();
            SkXs    = S' * obj.k(obj.X, obj.Xs);
            t_pred0 = toc(t_pred0);
                        
            t1 = tic(); % measure time
            S       = obj.K * S; % save memory by writing into the same variable
            Y       = S; % copy by reference
            clear S;
            Phib    = Y' * obj.y;
            ...
            [t_pred, ymu_hat, ys2_hat, alpha_small] = obj.predict(j, cholSY, L, Phib, SkXs, t_pred0);
    ...
    methods 
        ...
        function [t_pred, ymu_hat, ys2_hat, alpha_small] = predict(obj, j, cholSY, L, Phib, SkXs, t_pred0)
            % predict
            t_pred = tic();
            if j < obj.first_catch
            ...
            t_pred  = toc(t_pred) + obj.time_kernel + t_pred0;
        end
    end
end
